...
def get_keywords():
    ...
    return keyword.split(',') if ',' in keyword else [keyword]

def get_search_volume(browser, keyword):
    results = []
    try:
        tab = browser.new_tab()
        url = f'https://www.spyfu.com/keyword/overview?vwot=aa&query={keyword.strip()}'
        ...
        search_volume = ele.text if ele else "N/A"
...
def main():
    ...
    try:
        ...
        for keyword in keywords:
            # Get SpyFu data
            result = get_search_volume(browser, keyword)
            print("\nSpyFu Result:")
            print(result)
            
            # Get Ahrefs data
            ahrefs_result = getahrefsv(keyword, browser)
            print('\nAhrefs Result:')
            print(ahrefs_result)
